...
class wifi_tx_hu(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "Wifi Tx Hu")
        Qt.QWidget.__init__(self)
        self.setWindowTitle("Wifi Tx Hu")
        ...
        self.settings = Qt.QSettings("GNU Radio", "wifi_tx_hu")
        ...
        # Blocks
        ##################################################
        self._tx_gain_range = Range(0, 1, 0.01, 0.01, 200)
        ...
        # Create the options list
        self._op_options = ['a', 'd', 'r', 'l', 's']
        # Create the labels list
        self._op_labels = ['a', 'd', 'r', 'l', 's']
        # Create the combo box
        # Create the radio buttons
        self._op_group_box = Qt.QGroupBox('op' + ": ")
        self._op_box = Qt.QHBoxLayout()
        ...
        self._op_button_group = variable_chooser_button_group()
        self._op_group_box.setLayout(self._op_box)
        for i, _label in enumerate(self._op_labels):
            radio_button = Qt.QRadioButton(_label)
            self._op_box.addWidget(radio_button)
            self._op_button_group.addButton(radio_button, i)
        self._op_callback = lambda i: Qt.QMetaObject.invokeMethod(self._op_button_group, "updateButtonChecked", Qt.Q_ARG("int", self._op_options.index(i)))
        self._op_callback(self.op)
        self._op_button_group.buttonClicked[int].connect(
            lambda i: self.set_op(self._op_options[i]))
        self.top_layout.addWidget(self._op_group_box)
        # Create the options list
        ...
        self._interval_range = Range(1, 10000, 1, 100, 200)
        ...
        self.uhd_usrp_sink_0_1.set_center_freq(uhd.tune_request(freq, rf_freq = freq - lo_offset, rf_freq_policy=uhd.tune_request.POLICY_MANUAL), 0)
        ...
        self._pdu_length_range = Range(0, 2500, 1, 100, 200)
        ...
        self._noise_voltage_range = Range(0, 1, 0.05, 0, 200)
        ...
        self.foo_packet_pad2_0 = foo.packet_pad2(False, True, 0.02, 100, 1000)
        ...
        self.blocks_message_strobe_0_0 = blocks.message_strobe(pmt.intern("".join(op for i in range(10))), interval)
        ...
        self.connect((self.foo_packet_pad2_0, 0), (self.uhd_usrp_sink_0_1, 0))
    ...
    def set_op(self, op):
        self.op = op
        self._op_callback(self.op)
        self.blocks_message_strobe_0_0.set_msg(pmt.intern("".join(self.op for i in range(10))))
    ...
    def set_lo_offset(self, lo_offset):
        ...
        self._lo_offset_callback(self.lo_offset)
        self.uhd_usrp_sink_0_1.set_center_freq(uhd.tune_request(self.freq, rf_freq = self.freq - self.lo_offset, rf_freq_policy=uhd.tune_request.POLICY_MANUAL), 0)
...
def main(top_block_cls=wifi_tx_hu, options=None):
