...
#define DEC_DIST 1500
#define MAX_ANG 2000
#define WINDOW_SIZE 30
#define MAX_DATA_POINTS 1000
#define MIN_TARGET_ANGLE 50.0
#define MAX_TARGET_ANGLE 90.0
#define FRONT_ANGLE 10.0
...
// float window_average(std::vector<float>& data, int window_size)
// {
//     double sum = 0.0;
//     int count = 0;
...
//         count++;
//     }
//     float window_avg = sum / count;

//     // 遍历数据并计算平均值
//     int i = data.size() - 1;
//     if (data[i] > AMBIGUITY_TH * window_avg)
//     {
//         // cout << "奇异值：" << data[i] << std::endl;
//         // cout << "平均值：" << window_avg << std::endl;
...
// void plot_data(int num_data_points, double &data[][])
// {
//     //  Gnuplot实时画图
//     FILE *gnuplotPipe = popen("gnuplot -persist", "w");
//     if (gnuplotPipe)
//     {
//         fprintf(gnuplotPipe, "set title 'PID Control Output'\n");
//         fprintf(gnuplotPipe, "set xlabel 'Target Distance'\n");
//         fprintf(gnuplotPipe, "set ylabel 'PID Output'\n");
//         fprintf(gnuplotPipe, "plot '-' with lines\n");
//         for (int i = (num_data_points > 100) ? (num_data_points - 100) : 0; i < num_data_points; i++) // 画出最后100个点
//         {
...
bool inCone(Point p) {
    ...
    return std::abs(angle) < 10 || std::abs(angle - 360) < 10;
}
...
double check_front_distance(const std::vector<Point>& points) {
    ...
    if (filtered_points.empty()) {
        std::cout << "No points in cone" << std::endl;
        return 0;
    }

    // 聚类算法，返回聚类中心坐标
    Point center = cluster(filtered_points);
    ...
    std::cout << "Target distance: " << min_distance << std::endl;
    return min_distance;
}
        ...
        if (SL_IS_OK(op_result)) {
            ...
            if (target_cnt > 10){
                ...
                if (history_dist.size() >= 1){
                    float his_avg = std::accumulate(history_dist.begin(), history_dist.end(), 0.0) / history_dist.size();
            ...
            bool dec_flag = (front_cnt >= 10 && front_dist <= DEC_DIST);
            ...
            outfile << (double)target_dist << ' ' << (double)output << ' ' << dec_flag << ' ' << brake_flag << std::endl;
            logfile << "Frame Over:" << (double)target_dist << ' ' << (double)output << ' ' << dec_flag << ' ' << brake_flag << std::endl;
            outfile.flush();
            outfile.close();
            printf("front_cnt: %d", front_cnt);
            printf("----target dist=%0.2f, target_cnt=%d, output=%0.2f, front_dist=%0.2f, dec=%d, brake=%d----\n", 
                target_dist, target_cnt, output, front_dist, dec_flag, brake_flag);
